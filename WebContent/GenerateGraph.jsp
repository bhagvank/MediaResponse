		...
		<div id="fh5co-main">
			...
			<div class="fh5co-cards">
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-md-3">
			<form action="DataChart.jsp" method="get">
<table>
<%
...
{	
	...
	{
		String columnType = "type"+i;
		columnTypes.add(request.getParameter(columnType));
	}
	session.setAttribute("types",columnTypes);
}
else
{
	columnTypes = (List<String>) session.getAttribute("types");
}
...
for(String value:header)
{
	...
	out.println("<th border=1>");
	if(!value.isEmpty())
	{
	 out.println(value+ "&nbsp");
	}
	out.println("</th>");
  columnTypes.add(request.getParameter("type"+i));
  i++;
}
out.println("<th border=1>");
...
for(List<String> row: rows)
{
	if(k!=0)
	{
	   ...
	   for(String value: row)
	   {
		   ...
		   List<String> columnError = validator.validateData(value, columnTypes.get(j));
		   if(columnError != null)
		   {
			//   out.println(columnError);
			   if(error == null)
			   {
				   error = new ArrayList();
				   error.add(columnError);   
			   }
			   else
			   {
				   error.add(columnError);
	   ...
	   if(error != null)
	   {
		  // out.println(error.size());
		   StringBuffer message = new StringBuffer();
		   for(List<String> rowError: error)
		   {
			 //  out.println(rowError.toString());
			   for(String columnError: rowError)
			   {
//				   out.println(columnError);
		   ...
		   out.println("<td border=1>");
		   out.println("<A HREF=EditData.jsp?rowId="+k+"> Edit </A>");
		   out.println("</td>");
	   }
	   else
	   {
